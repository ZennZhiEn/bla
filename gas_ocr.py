...
def convert_pdf_to_image(pdf_file_path):
    ...
    images[0].save(f'page_gas{image_counter}.jpg', 'JPEG')
    image_counter += 1
...
def run_ocr_and_save_to_excel():
    ...
    for pdf_file_path in pdf_file_paths:
        ...
        image_path = f'page_gas{image_counter-1}.jpg'
        ...
        for idx, rectangle in enumerate(rectangles, start=1):
            ...
            cv2.imwrite(f'ext_imgas{idx}.jpg', open_cv_image)

            cropped_image_path = f'ext_imgas{idx}.jpg'
...
def create_excel_with_header(all_data, custom_text):
    ...
    column_headers = [
        ['No.'], ['Unit'], ['No.Akaun'], ['No.Invois'], ['Tarikh Bacaan Dahulu'], 
        ['Tarikh Bacaan Semasa'],['Jumlah (RM)'], ['Tunggakan (RM)'], ['Jumlah Perlu Dibayar (RM)']
    ]

# Merge the row before the column headers from column 1 to column 9
 ...
 # Insert the custom text
    ...
    col3_format = openpyxl.styles.NamedStyle(name='col3_format', number_format='00000000000')
    col4_format = openpyxl.styles.NamedStyle(name='col4_format', number_format='00000000')

# Apply the column formats to the corresponding columns
    sheet.column_dimensions['C'].number_format = '00000000000'
    sheet.column_dimensions['D'].number_format = '00000000'
 ...
 # Function to apply border to a range of cells
    ...
    for file_data in all_data:
        ...
        for col_idx, cell_data in enumerate(file_data, start=2):
            ...
            # If the cell is in column 7, 8, or 9, try to convert the value to float
            if col_idx in (7, 8, 9):
                try:
                    numeric_value = locale.atof(cell_data_str.replace(',', ''))
                    cell.value = numeric_value
                    ...
                    # Update the total_column_7, total_column_8, and total_column_9
                    ...
                    if col_idx == 7 and numeric_value is not None:
                        ...
                        total_column_9 += max(0, numeric_value) if numeric_value is not None else 0
                except ValueError:
                    cell.value = cell_data_str
            else:
                cell.font = font

   # Apply border to all cells with OCR data (column 1 to 9), column headers, total cells, and the total number
            if has_ocr_data or row_idx == 2 or row_idx == row_idx:  # row_idx == row_idx refers to the total row
                apply_border(row_idx, row_idx, 1, 9)
  ...
  # Apply fill to the row based on whether it has OCR data or not
        if has_ocr_data:
            ...
            for col in sheet.iter_cols(min_col=1, max_col=9, min_row=row_idx, max_row=row_idx):
                for cell in col:
...
# Apply border to the column headers, total cells, and last rows of columns 7, 8, and 9
    apply_border(2, 2, 1, 9)  # Column headers
...
# Create the GUI
root = tk.Tk()
root.title("GAS MALAYSIA OCR 2.0")
...
label_custom_text = tk.Label(root, text="Title:[e.g.: BEV GAS MALAYSIA JULY 2023 (VACANT UNIT)]", font=label_font, bg=label_bg_color, fg=label_text_color)
