...
def convert_pdf_to_image(pdf_file_path):
    ...
    images[0].save(f'page_water{image_counter}.jpg', 'JPEG')
    image_counter += 1
...
def run_ocr_and_save_to_excel():
    ...
    for pdf_file_path in pdf_file_paths:
        ...
        image_path = f'page_water{image_counter-1}.jpg'
        ...
        for idx, rectangle in enumerate(rectangles, start=1):
            ...
            cv2.imwrite(f'ext_imwater{idx}.jpg', open_cv_image)

            cropped_image_path = f'ext_imwater{idx}.jpg'
...
def create_excel_with_header(all_data, custom_text):
    ...
    column_headers = [
        ...
        ['Tarikh Akhir'],['Baki Belum Dijelaskan (RM)'], ['Bil Semasa (RM)'], ['Jumlah Perlu Dibayar (RM)']
    ]
...
# Merge the row before the column headers from column 1 to column 9
 ...
 # Insert the custom text
    ...
    col3_format = openpyxl.styles.NamedStyle(name='col3_format', number_format='0000000000')
...
# Apply the column formats to the corresponding columns
    sheet.column_dimensions['C'].number_format = '0000000000'
...
# Create the GUI
root = tk.Tk()
root.title("AIR SELANGOR OCR 2.0")
...
label_custom_text = tk.Label(root, text="Title:[e.g.: BEV AIR SELANGOR JULY 2023 (VACANT UNIT)]", font=label_font, bg=label_bg_color, fg=label_text_color)
